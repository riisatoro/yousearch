...
class Video(models.Model):
	vid_id = models.CharField(unique=True, max_length=100)
	...
	def __str__(self):
		return "{0} {1} {2} {3} {4} {5}".format(
			self.vid_id, self.name, self.link, self.iframe, self.date, self.preview)
...
class UserLikedList(models.Model):
	...
	def __str__(self):
		return "{0} {1}".format(self.user, self.video)
...
class RequestVideo(models.Model):
	...
	def __str__(self):
		return "{0} {1}".format(self.request, self.video)